# CLAUDE.md — archived "Current State" sections (pre-0.3.0)

These sections were moved verbatim out of `CLAUDE.md` on 2026-09-03 when it was restructured (see the single *Current state* section there and `CHANGELOG.md` for 0.3.0). They are historical: file names, coverage numbers and PR references are as of the date in each heading and are not maintained.

Personal paths that appeared in the original *Sibling Project* / *Smoke Test* sections have been replaced with `<local path>`.
...
VXD (vortex-dispatch) at `<local path>/vortex-dispatch` is the CLI-only variant (no native Gemma runtime). Shares: artifact store, scratchboard, DAG export, criteria, event store patterns. Does NOT share: semaphore, native runtime, controller, event bus.

## Smoke Test

Test project at `<local path>/nxd-smoke-test` with `nxd.yaml` configured for `gemma4:e4b` via Ollama. Clear state before re-running:
```bash
kill <stale-pid>
