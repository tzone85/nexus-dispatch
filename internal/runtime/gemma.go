		...
		for _, tc := range resp.ToolCalls {
			...
			if tc.Name == "task_complete" {
				...
				if err := json.Unmarshal(tc.Arguments, &args); err != nil {
					...
					log.Printf("[native-runtime] %q: task_complete malformed JSON: %v", g.StoryID, err)
					break
				}
				...
				if len(g.Criteria) > 0 {
					...
					if !criteria.AllPassed(criteriaResult) {
						...
						if criteriaRejections > maxRetries {
							log.Printf("[native-runtime] %q: criteria rejection budget exhausted (%d/%d), escalating",
						...
						log.Printf("[native-runtime] %q: task_complete rejected (%d/%d): %q",
...
func (g *GemmaRuntime) execReadFile(call llm.ToolCall, workDir string) llm.ToolCallResult {
	...
	if err != nil {
		log.Printf("[native-runtime] %q: read_file %q rejected: %s", g.StoryID, args.Path, rejectionDetail(err))
		result.IsError = true
	...
	if err != nil {
		...
		if !errors.Is(err, fs.ErrNotExist) {
			log.Printf("[native-runtime] %q: read_file %q: read error: %q", g.StoryID, args.Path, err.Error())
		}
		result.IsError = true
...
func (g *GemmaRuntime) execWriteFile(call llm.ToolCall, workDir string) llm.ToolCallResult {
	...
	absPath, err := safePath(args.Path, workDir)
	if err != nil {
		log.Printf("[native-runtime] %q: write_file %q rejected: %s", g.StoryID, args.Path, rejectionDetail(err))
		result.IsError = true
		result.Content = err.Error()
		return result
	}
	...
	if err := os.MkdirAll(dir, 0o755); err != nil {
		log.Printf("[native-runtime] %q: write_file %q: mkdir error: %q", g.StoryID, args.Path, err.Error())
		result.IsError = true
	...
	if err := os.WriteFile(absPath, []byte(args.Content), 0o644); err != nil {
		log.Printf("[native-runtime] %q: write_file %q: write error: %q", g.StoryID, args.Path, err.Error())
		result.IsError = true
...
func (g *GemmaRuntime) execEditFile(call llm.ToolCall, workDir string) llm.ToolCallResult {
	...
	absPath, err := safePath(args.Path, workDir)
	if err != nil {
		// rejectionDetail: the agent gets the path-relative reason, the
		// operator log the real one.
		log.Printf("[native-runtime] %q: edit_file %q rejected: %s", g.StoryID, args.Path, rejectionDetail(err))
		result.IsError = true
		result.Content = err.Error()
		return result
	}
	...
	if err != nil {
		if !errors.Is(err, fs.ErrNotExist) {
			log.Printf("[native-runtime] %q: edit_file %q: read error: %q", g.StoryID, args.Path, err.Error())
		}
		result.IsError = true
		result.Content = fmt.Sprintf("read error: %v", errReason(err))
		return result
	...
	if err := os.WriteFile(absPath, []byte(updated), 0o644); err != nil {
		log.Printf("[native-runtime] %q: edit_file %q: write error: %q", g.StoryID, args.Path, err.Error())
		result.IsError = true
		result.Content = fmt.Sprintf("write error: %v", errReason(err))
		return result
	}
