...
// TestRejectsZ_ExitCode129: git's parse-options layer translates its message
// ("Fehler: Unbekannter Schalter `z'"), so the English text is the second
// check, not the first. Exit 129 is what actually decides.
func TestRejectsZ_ExitCode129(t *testing.T) {
	bin := t.TempDir()
	script := "#!/bin/sh\necho 'Fehler: Unbekannter Schalter `z'\\''' >&2\nexit 129\n"
	if err := os.WriteFile(filepath.Join(bin, "git"), []byte(script), 0o700); err != nil {
		t.Fatal(err)
	}
	cmd := exec.Command(filepath.Join(bin, "git"), "worktree", "list", "--porcelain", "-z")
	out, err := cmd.CombinedOutput()
	if err == nil {
		t.Fatalf("the shim must fail, got %q", out)
	}
	wrapped := fmt.Errorf("%w (%s)", err, strings.TrimSpace(string(out)))
	if strings.Contains(strings.ToLower(wrapped.Error()), "unknown switch") {
		t.Fatalf("this case exists for a message the text match does NOT catch, got %v", wrapped)
	}
	if !rejectsZ(wrapped) {
		t.Fatalf("exit 129 is the parse-options usage error and must be recognised: %v", wrapped)
	}
}
