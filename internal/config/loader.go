...
// DefaultYAMLFor builds the default configuration tailored to the project
// found in dir: success criteria and allowlist entries match the detected
// toolchain instead of assuming Go. Wrong-language criteria are a hard
// pipeline failure on every story — a Swift repo gated on `go build` can
...
func DefaultYAMLFor(dir string) ([]byte, string, error) {
	...
	if len(profile.AllowlistExtras) > 0 {
		for name, rt := range cfg.Runtimes {
			rt.CommandAllowlist = append(rt.CommandAllowlist, profile.AllowlistExtras...)
			cfg.Runtimes[name] = rt
		}
	}
	data, err := yaml.Marshal(cfg)
	if err != nil {
		return nil, profile.Label, fmt.Errorf("marshalling default config: %w", err)
	}

	header := []byte("# NXD configuration — generated by 'nxd init'\n" +
		"# Success criteria generated for detected project type: " + profile.Label + "\n" +
		"# See https://github.com/tzone85/nexus-dispatch for documentation.\n\n")
	return append(header, data...), profile.Label, nil
}
