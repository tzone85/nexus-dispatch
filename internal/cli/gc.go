...
func runGC(cmd *cobra.Command, _ []string) error {
	...
	// gc removes worktrees and branches and appends events, so like archive
	// it takes the pipeline lock: a live nxd resume may be working in those
	// worktrees and projecting concurrently.
	lock, err := engine.AcquireLock(expandHome(s.Config.Workspace.StateDir))
	if err != nil {
		// Same shape as archive: the operator's first question is which
		// process holds it.
...
// DeleteBranch takes the shared path (branch_cleanup.go): a worktree that has
// the branch checked out is removed first, because git refuses `branch -D`
// otherwise. It says which worktree went — `worktree remove --force` discards
// uncommitted edits there, and "Cleaned up N branches." does not mention a
// directory. Nothing to remove comes back as errAlreadyClean, which the
// reaper never sees: it only calls this when BranchExists said yes.
func (g *cliGitCleanupOps) DeleteBranch(repoDir, branch string) error {
	removed, err := removeBranchAndWorktree(repoDir, branch)
	if removed != "" {
